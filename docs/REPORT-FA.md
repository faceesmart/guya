...
۳. تحلیل نیازمندی‌ها: کاربران، موارد کاربرد، گشتی در یک روز استفاده، نیازمندی‌های کارکردی، غیرکارکردی و ایمنی، محدودیت‌ها، محدوده
...
### ۳.۳ گشتی در یک روز استفاده

قضاوت دربارهٔ موارد کاربرد بالا وقتی آسان‌تر است که به شکل یک روز پیوسته دیده شوند. آنچه در ادامه می‌آید مسیر مورد نظر برای کاربر اصلی است، پس از آن‌که کمک‌کننده گویا را نصب کرده است؛ هر متنی که از صفحه نقل شده، همان است که نرم‌افزار نشان می‌دهد.

**صبح.** روی Guya دوبار کلیک می‌کند. پنل کنترل باز می‌شود، دکمهٔ روشن/خاموش آن سبزآبی می‌شود، خط وضعیت «Guya is on» را نشان می‌دهد و یک دایرهٔ خاکستری کوچک بالای صفحه ظاهر می‌شود. دیگر به پنل دست نمی‌زند؛ می‌توان آن را کمینه کرد. Word را باز می‌کند، مکان‌نما را می‌گذارد، کلید Right Option را نگه می‌دارد و یک جملهٔ فارسی را شروع می‌کند. دایره به یک پیل باز می‌شود، قرمز می‌شود و «● Dictation listening…» و پس از آن اولین واژه‌ها را، همان‌طور که تشخیص داده می‌شوند، نشان می‌دهد. وقتی کلید را رها می‌کند پیل آبی («Processing…») و سپس سبز («✓ Done») می‌شود و جمله، با فاصله‌گذاری فارسی و همان شکل‌های محاوره‌ای که گفته بود، در محل مکان‌نما ظاهر می‌شود. جمله به جمله ادامه می‌دهد. چیزی از رایانه بیرون نرفته است؛ مدل روی همان لپ‌تاپ اجرا می‌شود.

**یک ایمیل انگلیسی.** باید به انگلیسی پاسخ دهد. روی کلید دستیار می‌گوید «switch to English»؛ پیل با صدا پاسخ می‌دهد «Switched to English.» و نشان از FA به EN تغییر می‌کند. پاسخ را به همان روش دیکته می‌کند و بعد «برو فارسی» می‌گوید تا برگردد. پاسخ‌های فارسی خوانده نمی‌شوند، پس این یکی روی پیل و در حباب زیر آن ظاهر می‌شود.
...
**پیدا کردن فایل هفتهٔ پیش.** کمی بعد می‌گوید «فایل گزارش رو باز کن». دو نامزد وجود دارد، پس پیل می‌پرسد و یک پنجرهٔ کوچک آن‌ها را فهرست می‌کند: «۱. گزارش هفتگی.docx در Documents» و «۲. گزارش.docx در Desktop»، با راهنمای «روی گزینه بزنید یا بگویید اول، دوم، سوم یا لغو». می‌گوید «دومی»؛ باز می‌شود. تصمیم می‌گیرد نام درست نیست و می‌گوید «اسمش رو بذار گزارش قدیمی». حباب می‌پرسد «نام گزارش.docx به گزارش قدیمی.docx تغییر کند؟ بگویید بله یا نه.» فقط پس از «بله»ی او فایل تغییر نام می‌دهد، و اگر فایلی با آن نام وجود داشت دستیار رد می‌کرد.
...
**وقتی چیزی خراب می‌شود.** به اشتباه روی پیل کلیک می‌کند و بعد جمله‌ای دیکته می‌کند: پیل «Copied! Press ⌘V to paste» را نشان می‌دهد، چون حالا خود گویا برنامهٔ جلو است و متن روی کلیپ‌بورد مانده است. آن را جای‌گذاری می‌کند. بار دیگر پیل می‌گوید «Too short — hold the key while you speak»، چون کلید را پیش از اولین واژه رها کرده بود. اگر چیزی خارج از فهرست می‌خواست، مثلاً «delete the old report»، حباب توضیح می‌داد که گویا هرگز فایلی را حذف نمی‌کند.

**شب.** Word و مرورگر را مثل همیشه می‌بندد، سپس روی دکمهٔ روشن/خاموش پنل کلیک می‌کند یا پنجرهٔ پنل را می‌بندد. پیل هم با آن ناپدید می‌شود؛ هیچ فرایندی میکروفون را نگه نمی‌دارد.

این گشت مبنای سناریوی نمایش در `docs/DEMO.md` و فهرست کارهای مطالعهٔ کاربری (پیوست ت) است.
...
گویا یک پروژهٔ کاربردی است، اما تقریباً هر بخش آن بر مطالب درس‌های مهندسی کامپیوتر تکیه دارد. جدول ۵.۱۰ مفاهیمی را که واقعاً به کار رفته‌اند، همراه با بخشی که هر یک در آن ظاهر می‌شود، فهرست می‌کند تا پیوند میان درس‌ها و کد صریح باشد.
...
| پردازش سیگنال | نمونه‌برداری با ۱۶ کیلوهرتز، PCM تک‌کانالهٔ ۱۶ بیتی؛ سطح RMS و بهره با برش؛ جلوی کار طیف‌نگار log-mel | ضبط صدا (۵.۳)؛ یکسان‌سازی بلندی (۵.۴)؛ ورودی Whisper (۲.۱) |
...
| امنیت | فهرست‌های مجاز به جای متن آزاد؛ محصور بودن مسیرها با حل پیوندهای نمادین؛ نبودِ اجرای شل؛ کمترین امتیاز | مدل ایمنی (۴.۷) |

*جدول ۵.۱۰. مفاهیم درسی و جای کاربرد آن‌ها.*

دو مورد از این‌ها توضیح بیشتری می‌خواهند. روش ارزیابی فصل ۷ (یک مجموعهٔ دیده‌نشدهٔ ثابت، تغییر یک متغیر در هر بار، معیارهای سطح پیکره با یکسان‌سازی یکسان در دو طرف) بخشی از مطالب درسی بود که پروژه را بیش از همه تغییر داد، چون دو تصمیمی را که با شهود گرفته شده بودند برگرداند. و مطالب سیستم‌عامل در یک برنامهٔ رومیزی بیش از حد انتظار مهم از آب درآمد: دو اشکال دشوار ترم، ویجت یتیمی که میکروفون را نگه داشته بود و برنامه‌ای که از Finder اجرا شده و به محیط خودش دسترسی نداشت، مسئله‌های مدل فرایند و مجوز بودند، نه مسئله‌های گفتار.
