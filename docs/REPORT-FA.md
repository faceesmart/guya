...
کاربر در بخش ۳.۱ به‌صورت رسمی‌تر توصیف شده است. نکتهٔ مهم این است که پروژه برای یک فرد واقعی ساخته شده و تصمیم‌های طراحی فصل ۴ با توجه به کارهایی که او در عمل با برنامه خواهد کرد سنجیده شده‌اند. او تا زمان نوشتن این گزارش هنوز از نرم‌افزار استفاده نکرده بود؛ جلسه با او آماده شده و در انتظار انجام است (بخش ۷.۹).
...
| NFR-10 | قابلیت حمل: یک پایگاه کد روی macOS و Windows اجرا می‌شود. | آداپتورهای سکو برای کلیدهای میانبر، هدف‌گیری پنجره، ضربه‌های کلید و گفتار (`actions/macos.py`، `actions/windows.py`، `platform_macos.py`)؛ آداپتور Windows نوشته شده و با mock کردن سیستم‌عامل آزمون واحد شده است، اما هنوز روی یک دستگاه Windows اجرا نشده است (بخش ۷.۹). |
...
آداپتور Windows بر اساس مستندات API نوشته شد و تنها آزمونی که آن را اجرا می‌کند، با فراخوانی‌های سیستم‌عاملِ جایگزین‌شده (mock) اجرا می‌شود. این آداپتور هنوز روی یک دستگاه Windows اجرا نشده است (بخش ۷.۹).
...
1. **جلسه با کاربر هدف.** پروتکل نوشته شده است (پیوست ت): دوازده کار زمان‌گیری‌شده، همراه با مقیاس کاربردپذیری سیستم ده‌سؤالی به فارسی و ضبط صدای او برای مجموعهٔ دقت. این جلسه رقم دقتی را که بیش از همه اهمیت دارد، اندازه‌گیری‌شده روی گفتار خود او، همراه با نمرهٔ کاربردپذیری به دست می‌دهد.
2. **اجرا روی Windows.** آداپتور، نصب‌کننده و اجراکننده وجود دارند؛ به یک بعدازظهر روی دستگاه Windows خانواده نیاز دارند، و باید انتظار اصلاح‌هایی را داشت، چون هیچ‌یک از این کدها هرگز روی Windows اجرا نشده است.
3. **یک صدای فارسی.** Piper از طریق sherpa-onnx روی این دستگاه با RTF ۰٫۰۶ اجرا می‌شود؛ وصل کردن آن به متد `speak` آداپتور macOS تغییر کوچکی است، و این‌که کیفیت صدا قابل قبول هست یا نه را کاربر باید داوری کند.
...
8. **یک بک‌اند سریع‌تر روی Apple silicon.** faster-whisper بک‌اند مبتنی بر Metal ندارد؛ whisper.cpp دارد. روی دستگاه توسعه، این می‌تواند RTF مدل `large-v3-turbo` را به بسیار کمتر از ۰٫۲ برساند و پیاده کردن موقت گفتار را روان‌تر کند.
