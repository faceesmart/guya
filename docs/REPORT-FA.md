...
پیشنهادنامهٔ تصویب‌شده (بخش ۵.۱۴ تاریخچهٔ آن را خلاصه می‌کند) پنج هدف را بیان می‌کند. این اهداف در اینجا تکرار می‌شوند، چون ارزیابی فصل ۷ حول آن‌ها سازمان یافته است.
...
1. ضریب زمان واقعی پیش‌آزمون، `rtf_base`، برابر است با زمان پیاده کردن کلیپ ۷ ثانیه‌ای همراه برنامه با مدل `tiny` تقسیم بر طول کلیپ (یک گذر گرم‌کردن، سپس بهترین از دو گذر زمان‌سنجی‌شده، با بازگشت دمایی خاموش).

2. برای هر مدل m، ضریب زمان واقعی از این رابطه پیش‌بینی می‌شود:

        predicted_rtf[m] = rtf_base × RELATIVE_COST[m]

3. نامزدها مدل‌های آفلاینی‌اند که پروفایلر برای این حافظه مجاز می‌داند و رتبهٔ دقتشان دست‌کم `FLOOR[language]` است.

4. دقیق‌ترین نامزدی که `predicted_rtf ≤ 1.0` داشته باشد انتخاب می‌شود.

5. اگر هیچ نامزدی نماند، مدل آنلاین پیشنهاد می‌شود.
...
![پیل هنگام گوش دادن روی کلید دستیار](img/pill-assistant-listening.jpg)

![پیل بیکار در حالت دوگانه با نشان OFFLINE](img/pill-idle-offline.jpg)

![پیل بیکار در حالت دوگانه با نشان ONLINE](img/pill-idle-online.jpg)

*شکل ۴.۸. پیل روی صفحه: در حال گوش دادن روی کلید دستیار (قرمز، با نقطهٔ تپنده)، و بیکار در حالت دوگانه با نشان‌های پشتیبان OFFLINE و ONLINE؛ نشان زبان EN را نشان می‌دهد.*
...
![حباب پاسخ با یک پرسش بله/نه](img/bubble-question.jpg)

*شکل ۴.۹. حباب پاسخ پس از ساخته شدن یک فایل: پیل کهربایی شکل کوتاه پرسش را نشان می‌دهد و حباب آن را به‌طور کامل همراه با دکمه‌های Yes و No تکرار می‌کند.*

![پنجرهٔ انتخاب نتیجه با سه مورد نزدیک](img/popup-choices.jpg)

*شکل ۴.۱۰. پنجرهٔ انتخاب نتیجه برای نامی گفتاری با سه مورد نزدیک؛ هر گزینه نام فایل را بالای پوشه‌اش نشان می‌دهد و پیل ابتدای پاسخ گفتاری را نشان می‌دهد.*
...
- `widget.lock` و `control-panel.lock`: قفل‌های تک‌نمونه‌ای
...
- `guya/`: بستهٔ برنامه (۲۴ ماژول، ۱۱٬۹۰۵ خط)
    - `widget.py`: فرایند پیل
    - `stt.py`: خط لولهٔ گفتار به متن
    - `wizard.py`، `profiler.py`، `benchmark.py`
    - `control_panel.py`، `runtime.py`، `config.py`
    - `logsetup.py`، `launcher_gen.py`، `platform_macos.py`
    - `cloud_engine.py`، `__main__.py`
    - `assistant/`
        - `parser.py`، `service.py`، `normalizer.py`
        - `context.py`، `models.py`
        - `commands_en.json`، `commands_fa.json`: بسته‌های عبارت (۹۳ + ۱۰۰ نمونه)
        - `actions/`: `common.py`، `macos.py`، `windows.py`
    - `assets/`: `bench_speech.wav` (کلیپ آزمایشی ۷ ثانیه‌ای)، فونت‌های Vazirmatn
- `eval/`: ابزار ارزیابی (۸ ماژول، ۱٬۲۴۰ خط)
- `tests/`: آزمون‌ها (۱۳۶ آزمون در ۱۰ فایل، ۱٬۵۹۸ خط)
...
- ریشهٔ مخزن، برای macOS: `Install Guya.command`، `install.sh`، `run.sh`
- ریشهٔ مخزن، برای Windows: `Install Guya.bat`، `install.bat`، `run.bat`
...
پروژه بین ژوئن و سپتامبر ۲۰۲۶ در چهار مرحله توسعه یافت: ویجت دیکته و ویزارد (ژوئن)، دستیار (ژوئیه تا اوت)، اسپرینت ارزیابی (۵ سپتامبر) و جلسه‌های آزمون دستی و گزارش (۹ تا ۱۳ سپتامبر)؛ جدول ۵.۹ نقاط عطف را می‌دهد. مخزن در زمان نوشتن این گزارش ۵۶ کامیت دارد؛ بیشتر کار در جلسه‌های طولانی انجام و هنگام رسیدن به هر نقطهٔ عطف کامیت شده است.
...
تغییرات تحلیل‌گر و ویجت در ۵ سپتامبر همان روز خط‌به‌خط بازبینی شد و هر نقص مشکوک پیش از آن‌که شمرده شود بازتولید شد. بازبینی نقص‌هایی در شش گروه پیدا کرد؛ جدول ۷.۷ آن‌ها را بر حسب نوع فهرست می‌کند.
...
پاسخ‌های او به ده گزارهٔ مقیاس کاربردپذیری سیستم ۴، ۱، ۵، ۲، ۴، ۱، ۴، ۱، ۳ و ۲ بود که نمرهٔ ۸۲٫۵ از ۱۰۰ می‌شود؛ میانگین برای نرم‌افزارها به‌طور کلی ۶۸ است [۲۰]. پاسخ‌های او به سه پرسش باز، به زبان خودش (با املای یکسان‌شده):
...
*جدول ث.۱. اصطلاح‌های به‌کاررفته در گزارش و معادل فارسی آن‌ها.*

## پیوست ج. کد برنامه

کد کامل برنامه عمومی است و در نشانی https://github.com/faceesmart/guya قرار دارد؛ نسخه‌ای که این گزارش شرح می‌دهد برچسب `v1.0` را دارد. این پیوست همهٔ فایل‌های بستهٔ برنامه را فهرست می‌کند تا خواننده بتواند کد پشت هر فصل را پیدا کند، و سپس تنها روالی را نشان می‌دهد که نیازمندی‌های ایمنی فصل ۳ بر آن استوار است. ابزار ارزیابی (`eval/`، هشت ماژول) و آزمون‌ها (`tests/`، ده فایل) در جدول ۵.۲، جدول ۵.۸ و پیوست الف شرح داده شده‌اند.

| فایل | خط | محتوا |
|---|---:|---|
| `guya/__main__.py` | ۸۷ | نقطهٔ ورود: اگر پیکربندی وجود نداشته باشد ویزارد را اجرا می‌کند، وگرنه ویجت را |
| `guya/widget.py` | ۲٬۸۰۸ | فرایند پیل: ضبط‌کننده، کلیدهای میانبر، پیل شناور، حباب پاسخ، پنجرهٔ نتایج، اتصال به دستیار |
| `guya/stt.py` | ۵۷۴ | خط لولهٔ گفتار به متن: بارگذاری مدل، حذف سکوت، رمزگشایی، پس‌پردازش؛ مشترک با ارزیابی |
| `guya/wizard.py` | ۲٬۱۲۱ | ویزارد راه‌اندازی دوزبانهٔ یازده‌صفحه‌ای |
| `guya/profiler.py` | ۲۳۳ | پروفایل دستگاه (پردازنده، حافظه، کارت گرافیک) و پیشنهاد مبتنی بر مشخصات |
| `guya/benchmark.py` | ۲۶۵ | بنچمارک گفتار روی خود دستگاه و پیشنهادی که از آن ساخته می‌شود |
| `guya/control_panel.py` | ۱٬۰۱۷ | پنجرهٔ روزمره: دکمهٔ روشن و خاموش، تنظیمات، راهنما، لاگ، نگهداری |
| `guya/config.py` | ۱۷۸ | فایل پیکربندی و مقدارهای پیش‌فرض آن |
| `guya/runtime.py` | ۶۶ | تبادل وضعیت مبتنی بر فایل میان پنل و ویجت |
| `guya/logsetup.py` | ۶۶ | یک تنظیم لاگ برای هر سه فرایند |
| `guya/launcher_gen.py` | ۱۸۴ | ساخت `Guya.app` و `Start Guya` |
| `guya/platform_macos.py` | ۴۲۷ | آداپتور macOS: کلیدهای میانبر، برنامهٔ جلو، چسباندن، مجوزها |
| `guya/cloud_engine.py` | ۱۳۶ | بک‌اند گفتار آنلاین |
| `guya/__init__.py` | ۴ | نام و نسخهٔ بسته |
| `guya/assistant/__init__.py` | ۹ | یادداشت بسته |
| `guya/assistant/parser.py` | ۱٬۱۲۱ | قصدها و آرگومان‌ها، فارسی و انگلیسی |
| `guya/assistant/service.py` | ۷۹۸ | مسیریابی قصد، زمینه، تأیید، ارجاع |
| `guya/assistant/normalizer.py` | ۲۸۷ | یکسان‌سازی متن و نام فایل گفتاری |
| `guya/assistant/context.py` | ۸۷ | زمینهٔ کوتاه‌مدت پشت «اسمش را عوض کن» |
| `guya/assistant/models.py` | ۴۶ | کلاس‌های دادهٔ مشترک تحلیل‌گر، سرویس و عمل‌ها |
| `guya/assistant/actions/__init__.py` | ۱۶ | انتخاب آداپتور سکوی در حال اجرا |
| `guya/assistant/actions/common.py` | ۵۱۳ | مرز ایمنی و عمل‌های فایل‌سیستم |
| `guya/assistant/actions/macos.py` | ۵۴۷ | macOS: برنامه‌ها، مرورگر، بازخورد گفتاری |
| `guya/assistant/actions/windows.py` | ۳۱۵ | Windows: برنامه‌ها، مرورگر، بازخورد گفتاری |
| `guya/assistant/commands_en.json`، `commands_fa.json` | – | بسته‌های عبارت، ۹۳ نمونهٔ انگلیسی و ۱۰۰ نمونهٔ فارسی |
| `guya/assets/bench_speech.wav` | – | کلیپ هفت‌ثانیه‌ای بنچمارک |
| `guya/assets/fonts/` | – | Vazirmatn، چهار وزن |
| `install.sh`، `install.bat`، `Install Guya.command`، `Install Guya.bat`، `run.sh`، `run.bat` | – | نصب‌کننده‌ها و اجراکننده‌ها برای macOS و Windows |

*جدول ج.۱. فایل‌های بستهٔ برنامه؛ جمع فایل‌های Python همان ۱۱٬۹۰۵ خط جدول ۵.۲ است.*

هر عمل فایلی پیش از آن‌که به دیسک دست بزند از یک بررسی می‌گذرد: مسیر resolve می‌شود تا `..` و پیوندهای نمادین نتوانند به بیرون اشاره کنند، و باید زیر یکی از پوشه‌های مجاز باشد؛ هر چیزی که resolve نشود بیرون از مرز به حساب می‌آید. عمل تغییر نام الگویی را نشان می‌دهد که همهٔ عمل‌ها دنبال می‌کنند: بررسی هم روی مبدأ و هم روی مقصد اجرا می‌شود، مقصدی که از قبل وجود دارد هرگز بازنویسی نمی‌شود، و هر رد شدن به جای بالا بردن استثنا یک پیام دوزبانه برمی‌گرداند. آزمون‌های `tests/test_assistant_actions.py` هر یک از این رد شدن‌ها را می‌آزمایند.
