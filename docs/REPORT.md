...
The user is described more formally in Section 3.1. The important point is that the project was built for a real person, and the design decisions in Chapter 4 were judged against what he would actually do with it. He had not yet used the software when this report was written; the session with him is prepared and pending (Section 7.9).
...
| O2 | A separate bilingual assistant for a limited set of desktop actions: creating, finding, opening and renaming files and folders, opening applications, saving or closing the active window, and safe web search. |  (and add below the table: 'Browser navigation was added to O2 during implementation; the proposal listed only web search.')
...
The repository holds 55 commits between 5 June and 13 September 2026, with the assistant added in August and the evaluation data and results in September (Section 5.14 gives the history).
...
*Table 2.1. Whisper model sizes. Parameter counts from the model card [1]; Table caption: 'download sizes are the float16 CTranslate2 conversions the wizard fetches; the weights are quantised to 8-bit when the model is loaded.' Column header: 'Download size (float16, MB)'..*
...
| NFR-10 | Portability: the same code base runs on macOS and Windows. | platform adapters for hotkeys, window targeting, keystrokes and speech (`actions/macos.py`, `actions/windows.py`, `platform_macos.py`); the Windows adapter is written and unit-tested with the operating system mocked, but has not yet been run on a Windows machine (Section 7.9). |
...
The project was developed between June and September 2026 in four phases: the dictation widget and wizard (June), the assistant (July to August), the evaluation sprint (5 September) and the manual test sessions and report (9 to 13 September); Table 5.9 gives the milestones. The repository has 55 commits at the time of writing, concentrated on eight working days, because most of the work was done in long sessions and committed when a milestone was reached.
...
Guya set out to give a person with a physical disability, for whom typing at length is difficult, a way to write and to carry out everyday file, application and browser tasks by voice, in Persian and English, without any paid service. The software does this on macOS: it installs with one double-click, is configured by a wizard that benchmarks the machine, and has been in the author's daily use throughout the term. It is 11,905 lines of Python in 24 modules, with a safety boundary that cannot delete or overwrite anything, 136 automated tests, and an evaluation harness that reproduces every number in this report.
...
1. **The session with the target user.** The protocol is written (Appendix D): twelve tasks, timed, with the ten-item System Usability Scale in Persian and a recording of his voice for the accuracy set. It will give the accuracy figure that matters most, measured on his own speech, together with the usability score.
2. **A run on Windows.** The adapter, installer and launcher exist; they need one afternoon on the family's Windows machine, and fixes should be expected, since none of this code has ever been executed on Windows.
3. **A Persian voice.** Piper through sherpa-onnx runs at 0.06 RTF on this machine; wiring it into the `speak` method of the macOS adapter is a small change, and the user should judge whether the voice quality is acceptable.
...
8. **A faster backend on Apple silicon.** faster-whisper has no Metal backend; whisper.cpp does. On the development machine, this could bring `large-v3-turbo` well under 0.2 RTF and make the partial transcription smoother.
